"""
Произведение чисел 26 × 63 × 78 × 14 = 1788696.
...
def max_mult_matrix(str_matrix: list, limit: int) -> dict:
    ...
    # Конвертируем в пригодный для работы формат, матрицу
    matrix = [i.split() for i in L]
    matrix = [[int(j) for j in i] for i in matrix]

    # Вспомогательная функция, что бы не дублировать код, т.к. для вертикали и горизонтали он идентичен
    def get_matrix_m_max(matrix):
        max_m = 0
        max_numbers = []
        for index, item in enumerate(matrix):
            for i in range(len(item) - 1 - limit):
                inline_m = reduce(lambda x, y: x*y, item[i:i+limit])
                if inline_m > max_m:
                    max_m = inline_m
                    max_numbers = item[i:i+limit]
        return max_m, max_numbers

    def get_matrix_l_dig_m_max():
        max_m = 0
        max_numbers = []
        for index, item in enumerate(matrix):
            for i in range(len(item) - 1 - limit):
    ...
    max_m_h, max_nubers_h = get_matrix_m_max(matrix)
    verticeal_matrix = list(zip(*matrix))
